...
User Function RelMapSe()
	...
	Private _nTamMaxPr		:= 40  // Tamanho máximo do nome do produto
...
Static Function ImpLogo(oPrinter, nLin, nPag)
	...
	//MAPA DE SEPARAÇÃO
	nLin += 35
	oPrinter:Say(nLin,350,"MAPA DE SEPARAÇÃO",oFont28N:oFont)	
...
Static Function ImpCab(oPrinter, nLin, nPag)
	...
	nLin += _nProxLin
	SayLabelValue( oPrinter, nLin, _nColInicio, "Número Único:", SC5->C5_NUM )
...
Static Function ImpProd(oPrinter, nLin, nPag, _nMaxLin)
	...
	EndIf
	DbSelectArea("SB1")
	SB1->(DbSetOrder(1)) // Filial + Pedido
	...
	While !SC6->(Eof()) .And. ;
		...
		// Código		
		oPrinter:Say(nLin, _nColInicio, SC6->C6_PRODUTO, oFont11:oFont)
		

		nProxCol += 75
		// Quebra descrição sem cortar palavra
		aLinhas := QuebraTexto(SC6->C6_DESCRI, _nTamMaxPr)
		// Produto (linha 1)
		...
		// Unidade
		oPrinter:Say(nLin, _nColInicio + nProxCol, SC6->C6_UM, oFont11:oFont)
		...
		oPrinter:Say(nLin, ;
			_nColInicio + nProxCol, ;
			Transform(SC6->C6_QTDVEN, "@E 999.999"), ;
			oFont11:oFont)
		...
		oPrinter:Say(nLin, ;
			_nColInicio + nProxCol, ;
			PadR(Posicione("SA5",2,xFilial('SA5')+SC6->C6_PRODUTO,'A5_CODPRF'), 18), ;
			oFont11:oFont)
...
Static Function GetEndEnt()
	...
	If (SC5->(C5_CLIENT + C5_LOJAENT) == SC5->(C5_CLIENTE + C5_LOJACLI))
		cEndEnt := SA1->A1_END + ", "  + SA1->A1_BAIRRO + " - CEP: " + Transform(SA1->A1_CEP, "@R 99999-999") + " - " + SA1->A1_MUN + " - " + SA1->A1_EST 
	Else
