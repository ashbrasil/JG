...
oBrowse := TWBrowse():New( 22 ,03, 620, 166, ,{'','Filial','Fil.Origem','Local','Tipo','Pedido','Orçamento','Qtde','Emissao','Vendedor','Cliente'},;
...
@ 007,090 Say POSICIONE("SB1",1,FWxfilial("SB1")+cGetProduto,"B1_DESC") Size 200,008 Font oFont10B COLOR CLR_BLACK PIXEL OF oDlg
...
Static Function RefreshBrw()
	...
	lProdOk := MsSeek(FWxfilial("SB1")+cGetProduto)
	...
	cQry := "SELECT "+CRLF
	cQry += "	CASE WHEN RES_FILIAL = "+ValToSql(cFilAnt)+" THEN 1 ELSE 2 END RES_ORDEM, "+CRLF
	cQry += "	RES_PRODUT, RES_LOCAL, RES_STATUS, RES_FILIAL, RES_FILORI, RES_TIPO, RES_PEDIDO, RES_NUMORC, RES_EMISSA, RES_QUANT, "+CRLF
	cQry += "	CASE "+CRLF
	cQry += "		WHEN RES_VEND IS NULL THEN ' ' "+CRLF
	cQry += "		WHEN A3_NOME IS NULL THEN RES_VEND "+CRLF
	cQry += "		ELSE RES_VEND || ' - ' || RTRIM(A3_NOME) "+CRLF
	cQry += "	END AS RES_VEND, "+CRLF
	cQry += "	CASE "+CRLF
	cQry += "		WHEN RES_CLIENT IS NULL THEN ' ' "+CRLF
	cQry += "		WHEN A1_NOME IS NULL THEN RES_CLIENT || '-' || RES_LOJA "+CRLF
	cQry += "		ELSE RES_CLIENT || '-' || RES_LOJA || ' - ' || RTRIM(A1_NOME) "+CRLF
	...
	cQry += "		CASE "+CRLF
	cQry += "			WHEN C0_FILRES = ' ' OR C0_FILRES = C0_FILIAL THEN "+ValToSql(Space(TamSx3("C0_FILIAL")[1]))+" "+CRLF
	cQry += "			ELSE C0_FILRES "+CRLF
	cQry += "		END AS RES_FILORI, "+CRLF
	cQry += "		'FAT' AS RES_TIPO, "+CRLF
	cQry += "		C0_DOCRES AS RES_PEDIDO, "+CRLF
	cQry += "		COALESCE(C6_NUMORC, "+ValToSql(Space(TamSX3("C6_NUMORC")[1]))+") AS RES_NUMORC, "+CRLF
	...
	cQry += "			WHEN C6_QTDVEN IS NOT NULL THEN "+CRLF
	...
	cQry += "				AND C9_PRODUTO = C0_PRODUTO AND C9_RESERVA = C0_NUM AND C9_NFISCAL = "+ValToSql(Space(TamSx3("C9_NFISCAL")[1]))+" "+CRLF
	cQry += "				AND C9_SERIENF = "+ValToSql(Space(TamSx3("C9_SERIENF")[1]))+" "+CRLF
	...
	cQry += "		CASE "+CRLF
	cQry += "			WHEN COALESCE(L1.L1_DOC,"+ValToSql(Space(TamSx3("L1_DOC")[1]))+") <> ' ' OR COALESCE(LX.L1_DOC,"+ValToSql(Space(TamSx3("L1_DOC")[1]))+") <> ' ' THEN 6 "+CRLF
	cQry += "			WHEN LX.L1_ORCRES IS NULL THEN "+CRLF
	cQry += "				CASE "+CRLF
	cQry += "					WHEN L2_ENTREGA IS NULL THEN 1 "+CRLF
	...
	cQry += "		C0_DOCRES AS RES_PEDIDO, "+CRLF
	cQry += "		"+ValToSql(Space(TamSX3("C6_NUMORC")[1]))+" AS RES_NUMORC, "+CRLF
	...
	cQry += "		"+ValToSql(Space(TamSx3("C9_FILIAL")[1]))+" AS RES_FILORI, "+CRLF
	cQry += "		'FAT' AS RES_TIPO, "+CRLF
	...
	cQry += "		AND C9_BLEST   = " + ValToSql(Space(TamSx3("C9_BLEST")[1])) + " "+CRLF
	cQry += "		AND C9_RESERVA = " + ValToSql(Space(TamSx3("C9_RESERVA")[1])) + " "+CRLF
	cQry += "		AND C9_NFISCAL = " + ValToSql(Space(TamSx3("C9_NFISCAL")[1])) + " "+CRLF
	cQry += "		AND C5_NOTA    = " + ValToSql(Space(TamSx3("C5_NOTA")[1]))+" "+CRLF
	cQry += "		AND F4_ESTOQUE = 'S' "+CRLF

	cQry += "	) RES "+CRLF
	cQry += "	LEFT JOIN " + RetSqlName("SA3") + " A3 "+CRLF
	cQry += "		ON A3.D_E_L_E_T_ <> '*' AND A3_FILIAL = " + ValToSql(FWxfilial("SA3")) + " AND RES_VEND = A3_COD "+CRLF
	cQry += "	LEFT JOIN " + RetSqlName("SA1") + " A1 "+CRLF
	cQry += "		ON A1.D_E_L_E_T_ <> '*' AND A1_FILIAL = " + ValToSql(FWxfilial("SA1")) + " AND A1_COD = RES_CLIENT AND A1_LOJA = RES_LOJA "+CRLF
	cQry += "ORDER BY "+CRLF
