...
User Function JG05A002(cNota, cSerie, cCliente, cLoja)
	...
	// 1. Envia o título para o borderô (fGeraBord também devolve, em aSEARecnos,
  	// os R_E_C_N_O_ da SEA recém-criados, necessários para a FINA713)
  	lBordOK1 := fGeraBord(cNota, cSerie, cCliente, cLoja, aSEARecnos)

	// 1. Envia o Titulo para o Borderô
	//lBordOK1 := fGeraBord(cNota, cSerie, cCliente, cLoja)
	If lBordOK1
	 	 // 2. Transmite o título para a TechFin e 3. aguarda a emissão/autorização do boleto
     	//	lComOK := fComTechFin(cNota, cSerie, cCliente, cLoja, @cPdfPath)
     	//	If !lComOK
      	//		MsgStop("Não foi possível concluir a comunicação com a TechFin. O boleto não será impresso.", "Erro")
      	//		cPdfPath := ""
    	//	EndIf
  	//Else
...
//Return cPdfPath
 		If Len(aSEARecnos) == 0
      	   MsgStop("Borderô gerado, mas nenhum título pendente de transmissão foi localizado na SEA (EA_CART='R' e EA_TRANSF<>'S'). O boleto não será impresso.", "Erro")
      	   Return aPdfUrls
    	EndIf
    	// 2. Transmite o(s) título(s) para o banco/TechFin (chamada SÍNCRONA) e
   		// 3. lê o resultado, que já vem pronto na SEA ao final da mesma chamada
    	lComOK := fComTechFin(aSEARecnos, aPdfUrls)
    	If !lComOK
      		MsgStop("Não foi possível concluir a comunicação com a TechFin. O boleto não será impresso.", "Erro")
    	EndIf
 	Else
    	MsgStop("Não foi possível gerar o borderô. O boleto não será impresso.", "Erro")
  	EndIf
// Retorna as URLs dos boletos autorizados pela TechFin (array vazio se algum passo falhou)
Return aPdfUrls

/*/{Protheus.doc} fGeraBord
Gera o borderô via ExecAuto (FINA060)
@param aSEARecnos Array (saída) recebe os R_E_C_N_O_ da SEA gerados para o título,
       necessários para a transmissão via FINA713 em fComTechFin.
/*/
Static Function fGeraBord(cNota, cSerie, cCliente, cLoja, aSEARecnos)
	Local aCab      := {}
	Local aItens    := {}
	Local cQuery    := ""
   ...
   // Alert('Após QueryE1 c/ Return')
	...
	Else
		...
		// Recupera os R_E_C_N_O_ da SEA recém-criados para este título, para
        // que fComTechFin possa chamar a FINA713 com a lista correta de recnos.
    	cQuery := "SELECT SEA.R_E_C_N_O_ AS RECSEA "
    	cQuery += "FROM " + RetSqlName("SEA") + " SEA "
    	cQuery += "WHERE SEA.EA_FILIAL  = '" + xFilial("SEA") + "' "
    	cQuery += "AND SEA.EA_PREFIXO   = '" + cSerie + "' "
    	cQuery += "AND SEA.EA_NUM       = '" + cNota + "' "
    	cQuery += "AND SEA.EA_CART      = 'R' "
    	cQuery += "AND SEA.EA_TRANSF    <> 'S' "
    	cQuery += "AND SEA.D_E_L_E_T_   = ' ' "
    	TCQuery cQuery New Alias (cAliasSEA)
    	While !(cAliasSEA)->(EoF())
      		aAdd(aSEARecnos, (cAliasSEA)->RECSEA)
      		(cAliasSEA)->(DbSkip())
    	EndDo
...
Static Function fComTechFin(cNota, cSerie, cCliente, cLoja)
  	Local aCab           := {}
  	Local aItens         := {}
  	Local lRet           := .T.
  	Local cDirTechFin    := AllTrim(SuperGetMV("MV_XDIRTEC",,"\SPOOL\")) // <-- ajustar parametro/caminho conforme ambiente
  	Local cMascara       := "Boleto_Bancario_" + AllTrim(cNota) + "*.PDF"
  	Local aArqTechFin    := {}
  	Local nTentativas    := 0
  	Local nMaxTentativas := 15 // 30 <-- ajustar conforme SLA de resposta da TechFin
  	Local nIntervalo     := 10 // 10 segundos entre tentativas
 	Local lAchou         := .F.
  	Private lMsErroAuto := .F.
  
  // ATENÇÃO: os campos AUT* abaixo seguem o padrão de rotina automática (ExecAuto)
  // da TOTVS e servem de PONTO DE PARTIDA. Antes de usar em produção, valide no
  // dicionário (SX3) deste ambiente os nomes de campo realmente exigidos pela
  // FINA713 e a operação correta (nOper) para a comunicação/transmissão do título.
  	aAdd(aCab, {"AUTFILIAL"  , xFilial("SE1")                            })
  	aAdd(aCab, {"AUTPREFIXO" , PadR(cSerie  , TamSX3("E1_PREFIXO")[1])   })
  	aAdd(aCab, {"AUTNUM"     , PadR(cNota   , TamSX3("E1_NUM")[1])       })
  	aAdd(aCab, {"AUTCLIENTE" , PadR(cCliente, TamSX3("E1_CLIENTE")[1])   })
  	aAdd(aCab, {"AUTLOJA"    , PadR(cLoja   , TamSX3("E1_LOJA")[1])      })
 	// Executa a rotina automática FINA713 (Comunicação Bancária / TechFin)
  	// nOper 3 = Inclusão/disparo da comunicação — confirmar com o suporte TOTVS
  	MSExecAuto({|a, b| FINA713(a, b)}, 3, {aCab, aItens})  	
	// conout sobre boleto não aparecer na techfin
	ConOut("[fComTechFin] NF="+AllTrim(cNota)+"/"+AllTrim(cSerie)+" - FINA713 lMsErroAuto="+cValToChar(lMsErroAuto))
	...
	MsgInfo("Comunicação com a TechFin disparada. Aguardando disponibilização do boleto...", "TechFin")
  	// Persiste a pesquisa no diretório até o título ficar disponível ou estourar o tempo limite
  	nTentativas := 0
 	lAchou      := .F.
	// conout fComTechFin aguardando boleto
 		ConOut("[fComTechFin] Aguardando boleto em: "+cDirTechFin+cMascara+" (max "+cValToChar(nMaxTentativas)+" tentativas de "+cValToChar(nIntervalo)+"s)")
 	 	
		While !lAchou .And. nTentativas < nMaxTentativas
    		aArqTechFin := Directory(cDirTechFin + cMascara)
	 	ConOut("[fComTechFin] Tentativa "+cValToChar(nTentativas+1)+"/"+cValToChar(nMaxTentativas)+" - arquivos encontrados: "+cValToChar(Len(aArqTechFin)))

   		    If Len(aArqTechFin) > 0
      	       lAchou := .T.
   			Else
      		   nTentativas++
      		   SysRefresh()
  	...
  	If !lAchou
    	lRet := .F.
	ConOut("[fComTechFin] TIMEOUT - boleto NF "+AllTrim(cNota)+" não encontrado em "+cDirTechFin+cMascara)


    	MsgAlert("Tempo limite excedido: o boleto do título " + AllTrim(cNota) + ;
      	" não foi disponibilizado pela TechFin em " + AllTrim(Str(nMaxTentativas * nIntervalo)) + " segundos.", "TechFin")
  	Else
    	MsgInfo("Boleto disponibilizado pela TechFin: " + cDirTechFin + aArqTechFin[1,1], "TechFin")
  	EndIf
Return lRet
